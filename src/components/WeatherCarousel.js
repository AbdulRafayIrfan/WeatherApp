    ...
    return (
      <motion.div className="carousel" whileTap={{ cursor: 'grabbing' }}>
        <motion.div drag="x" dragConstraints={{ right: 0, left: -208 }} className="inner-carousel">
          ...
          {
                        units === 'metric'
                          ? this.fullDayForecast.map((value, index) => <WeatherCarouselElement key={index} time={value.dt_txt} temp={value.main.temp} icon={value.weather[0].icon} />)
                          : this.fullDayForecastF.map((value, index) => <WeatherCarouselElement key={index} time={value.dt_txt} temp={value.main.temp} icon={value.weather[0].icon} />)
                    }
        </motion.div>
      </motion.div>
    );
