...
class WeatherScrollable extends Component {
    ...
    render() {
        const { current: currentC, currentF, forecast: forecastC, forecastF } = this.context.response;
        ...
        return (
            <div className="weather-scrollable" ref={this.scrollRef}>
                <div className="weather-desc">
                    <p>Today: {current.weather[0].description} conditions with a heat index of {Math.round(current.main.feels_like)}°.
                    The high will be {Math.ceil(current.main.temp_max)}° alongside a low of {Math.floor(current.main.temp_min)}°</p>
                </div>
                <div className="weather-stats">
                    ...
                    <div>
                        <p>Humidity</p>
                        <h3>{current.main.humidity}% <FontAwesomeIcon onClick={this.scrollDown} className={"chevrons "} pull="right" size="xs" icon={faChevronDown}/></h3>
                    </div>
                    <div>
                        <p>Wind</p>
                    ...
                    <div>
                        <p>Feels like</p>
                        <h3>{Math.round(current.main.feels_like)}°</h3>
                    </div>
                    <div>
                        <p>Pressure</p>
                    ...
                    <div>
                        <p>Cloudiness</p>
                        <h3>{current.clouds.all}% <FontAwesomeIcon onClick={this.scrollUp} className={"chevrons "} size="xs" pull="right" icon={faChevronUp}/></h3>
                    </div>
                </div>
            </div>
        );
